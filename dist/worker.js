var L={title:"Nika Net",host:"nika.example.workers.dev",sni:"www.speedtest.net",wsPath:"/nika-ws",cleanIps:["1.0.0.1","104.16.132.229","188.114.96.9"],protocols:{vless:!0,trojan:!0,warp:!0},adminPassHash:null,secretPath:"nika-admin",sessionSecret:""};var q=new Map,et=new Map;async function Nt(e,t){try{let r=await e.NIKA_DB.prepare("SELECT value FROM kv WHERE key = ?1").bind(t).first();return r?r.value:null}catch{return null}}async function Dt(e,t,r){try{await e.NIKA_DB.prepare("INSERT INTO kv (key, value) VALUES (?1, ?2) ON CONFLICT(key) DO UPDATE SET value = excluded.value").bind(t,r).run()}catch{}}async function N(e,t){if(q.has(t))return q.get(t);let r=null;return e.NIKA_DB?r=await Nt(e,t):e.NIKA_KV?r=await e.NIKA_KV.get(t):r=et.get(t)??null,r!==null&&q.set(t,r),r}async function U(e,t,r){q.set(t,r),e.NIKA_DB?await Dt(e,t,r):e.NIKA_KV?await e.NIKA_KV.put(t,r):et.set(t,r)}async function rt(e){let t=await N(e,"settings"),r={...L};if(t)try{Object.assign(r,JSON.parse(t))}catch{}return r.sessionSecret||(r.sessionSecret=crypto.randomUUID(),await U(e,"settings",JSON.stringify(r))),r}async function W(e,t){await U(e,"settings",JSON.stringify(t))}async function b(e){let t=await N(e,"users");if(!t)return[];try{return JSON.parse(t)}catch{return[]}}async function v(e,t){await U(e,"users",JSON.stringify(t))}async function R(e,t){let r=await N(e,t),a=parseInt(r||"0",10);return isNaN(a)?0:a}async function P(e,t,r){let n=await R(e,t)+r;return await U(e,t,String(n)),n}async function M(e,t){let r=await N(e,t);if(!r)return null;try{return JSON.parse(r)}catch{return null}}async function nt(e,t,r){await U(e,t,JSON.stringify(r))}var G=new TextEncoder;async function J(e){let t=await crypto.subtle.digest("SHA-256",G.encode(e));return[...new Uint8Array(t)].map(r=>r.toString(16).padStart(2,"0")).join("")}function st(){return[...crypto.getRandomValues(new Uint8Array(24))].map(t=>t.toString(16).padStart(2,"0")).join("")}async function j(e,t){let r=await crypto.subtle.importKey("raw",G.encode(e),{name:"HMAC",hash:"SHA-256"},!1,["sign"]),a=await crypto.subtle.sign("HMAC",r,G.encode(t));return t+"."+[...new Uint8Array(a)].map(n=>n.toString(16).padStart(2,"0")).join("")}async function Ot(e,t){let r=t.lastIndexOf(".");if(r<0)return!1;let a=t.slice(0,r),c=(await j(e,a)).split(".")[1],o=t.slice(r+1);if(c.length!==o.length)return!1;let s=0;for(let i=0;i<c.length;i++)s|=c.charCodeAt(i)^o.charCodeAt(i);return s===0}async function ot(e,t){let r=(e.headers.get("Cookie")||"").split(";").map(n=>n.trim()).find(n=>n.startsWith("nika_session="));if(!r)return!1;let a=decodeURIComponent(r.split("=").slice(1).join("="));return Ot(t.sessionSecret,a)}var z="nika_session";function D(e){let t=(e.cleanIps||[]).filter(Boolean);return t.length?t[Math.floor(Math.random()*t.length)]:e.host}function Kt(e,t){let r=D(t),a=new URLSearchParams({encryption:"none",security:"tls",sni:t.sni,fp:"chrome",type:"ws",host:t.sni,path:t.wsPath+"?ed=2048"});return`vless://${e.uuid}@${r}:443?${a.toString()}#NikaNet-${encodeURIComponent(e.name)}`}function Vt(e,t){let r=D(t),a=new URLSearchParams({security:"tls",sni:t.sni,fp:"chrome",type:"ws",host:t.sni,path:t.wsPath+"?ed=2048"});return`trojan://${e.password}@${r}:443?${a.toString()}#NikaNet-${encodeURIComponent(e.name)}`}function O(e,t){let r=[];return t.protocols.vless&&r.push(Kt(e,t)),t.protocols.trojan&&r.push(Vt(e,t)),btoa(r.join(`
`)).replace(/=+$/,"")}function Z(e,t){let r=D(t),a=[],n=`# Nika Net \u2014 ${e.name}
mixed-port: 7890
allow-lan: false
mode: rule
log-level: info
...
proxies:
`;return t.protocols.vless&&(a.push("NikaNet-VLESS"),n+=`  - name: "NikaNet-VLESS"
    type: vless
    server: ${r}
    port: 443
    uuid: ${e.uuid}
    ...
    ws-opts:
      ...
      headers: { Host: "${t.sni}" }
`),t.protocols.trojan&&(a.push("NikaNet-Trojan"),n+=`  - name: "NikaNet-Trojan"
    type: trojan
    server: ${r}
    port: 443
    password: ${e.password}
...
`),n+=`proxy-groups:
  - name: "NikaNet"
    type: select
    proxies: [${a.map(c=>`"${c}"`).join(", ")}]
`,n+=`rules:
  - GEOIP,IR,DIRECT
  - MATCH,NikaNet
`,n}function X(e,t){let r=D(t),a=[],n=[];t.protocols.vless&&(n.push("NikaNet-VLESS"),a.push({tag:"NikaNet-VLESS",type:"vless",server:r,server_port:443,uuid:e.uuid,network:"ws",tls:{enabled:!0,server_name:t.sni,utls:{enabled:!0,fingerprint:"chrome"}},transport:{type:"ws",path:t.wsPath+"?ed=2048",headers:{Host:t.sni}}})),t.protocols.trojan&&(n.push("NikaNet-Trojan"),a.push({tag:"NikaNet-Trojan",type:"trojan",server:r,server_port:443,password:e.password,network:"ws",tls:{enabled:!0,server_name:t.sni,utls:{enabled:!0,fingerprint:"chrome"}},transport:{type:"ws",path:t.wsPath+"?ed=2048",headers:{Host:t.sni}}}));let c={log:{level:"info"},dns:{servers:[{tag:"cf",address:"https://1.1.1.1/dns-query",detour:"select"}]},outbounds:a.concat([{tag:"select",type:"selector",outbounds:n},{tag:"direct",type:"direct"}]),route:{rules:[{geoip:"ir",outbound:"direct"}],final:"select"}};return JSON.stringify(c,null,2)}function it(e){return`[Interface]
PrivateKey = ${e.password.slice(0,43)}
...
`}var E=new Map,ct=new Map,lt=0,B=()=>{let e=new Date,t=String(e.getMonth()+1).padStart(2,"0"),r=String(e.getDate()).padStart(2,"0");return`${e.getFullYear()}-${t}-${r}`};function pt(){E.set("req:total",(E.get("req:total")||0)+1),E.set("req:"+B(),(E.get("req:"+B())||0)+1)}async function dt(e){let t=Date.now();if(!(t-lt<15e3)){lt=t;for(let[r,a]of E)a&&await P(e,r,a);E.clear()}}async function gt(e){return await R(e,"req:"+B())+(E.get("req:"+B())||0)}async function ut(e){return await R(e,"req:total")+(E.get("req:total")||0)}async function C(e,t,r,a){if(!r&&!a)return;let n=await b(e),c=n.find(s=>s.id===t);c&&(c.used=(c.used||0)+(r+a)/1e9,await v(e,n)),await P(e,"traffic:"+B(),r+a);let o=ct.get(t)||0;Date.now()-o>6e4&&(ct.set(t,Date.now()),await I(e,{icon:"\u{1F4E1}",text:`\u0627\u062A\u0635\u0627\u0644 \u062C\u062F\u06CC\u062F${c?` \u2014 ${c.name}`:""}`,time:Date.now()}))}async function ft(e){let t=[];for(let r=6;r>=0;r--){let a=new Date;a.setDate(a.getDate()-r);let n=String(a.getMonth()+1).padStart(2,"0"),c=String(a.getDate()).padStart(2,"0"),o=`${a.getFullYear()}-${n}-${c}`,s=await R(e,"traffic:"+o);t.push({date:o.slice(5),gb:Math.round(s/1e9*100)/100})}return t}async function I(e,t){let r=await M(e,"activity")||[];r.unshift(t),r.length>40&&(r.length=40),await nt(e,"activity",r)}async function mt(e){return await M(e,"activity")||[]}import{connect as Tt}from"cloudflare:sockets";function Y(e,t,r){let a=null;try{t&&(a=Uint8Array.from(atob(t),s=>s.charCodeAt(0)))}catch{a=null}let n=null,c=new Promise(s=>n=s);return e.addEventListener("message",()=>{n&&(n(),n=null)}),e.addEventListener("error",()=>{n&&(n(),n=null)}),{stream:new ReadableStream({start(s){let i=p=>{let l=p.data;l instanceof ArrayBuffer?s.enqueue(new Uint8Array(l)):Array.isArray(l)?s.enqueue(new Uint8Array(l)):typeof l=="string"&&s.enqueue(new TextEncoder().encode(l))};e.addEventListener("message",i),e.addEventListener("close",()=>{try{s.close()}catch{}}),e.addEventListener("error",()=>{try{s.error(new Error("ws error"))}catch{}})},async pull(){await c},cancel(){try{e.close()}catch{}}}),removeEarlyData:()=>{let s=a;return a=null,s}}}function K(e){let t=new WebSocketPair,[r,a]=Object.values(t);return a.accept(),a}function g(e,t=200){return new Response(JSON.stringify(e),{status:t,headers:{"content-type":"application/json; charset=utf-8","access-control-allow-origin":"*"}})}async function xt(e,t,r,a){let n=K(e);n.binaryType="arraybuffer";let c=e.headers.get("sec-websocket-protocol")||"",{stream:o,removeEarlyData:s}=Y(n,c,()=>{}),i=d=>{try{n.send(d)}catch{}},p=0,l=0;return o.pipeTo(new WritableStream({async write(d){let u=null;try{let f=s(),A=f&&f.length?Wt(f,d):d,x=Pt(A,t);if(!x){n.close();return}if(u=Tt({hostname:x.address,port:x.port}),i(new Uint8Array([0,0])),x.payload.length){let h=u.writable.getWriter();await h.write(x.payload),h.releaseLock(),p+=x.payload.length}o.pipeTo(new WritableStream({async write(h){p+=h.byteLength;let w=u.writable.getWriter();try{await w.write(h)}finally{w.releaseLock()}}})).catch(()=>{try{u.close()}catch{}}),u.readable.pipeTo(new WritableStream({write(h){l+=h.byteLength,i(h)}})).catch(()=>{try{n.close()}catch{}}).finally(()=>{ht(a,t.id,p,l)})}catch{try{u?.close()}catch{}try{n.close()}catch{}ht(a,t.id,p,l)}}})).catch(()=>{try{n.close()}catch{}}),new Response(null,{status:101,webSocket:n})}function ht(e,t,r,a){C(e,t,r,a).catch(()=>{})}function Wt(e,t){let r=new Uint8Array(e.length+t.length);return r.set(e),r.set(t,e.length),r}function Pt(e,t){try{let r=0;if(e[r]!==0)return null;r+=1;let a=Mt(e.slice(r,r+16));if(r+=16,a.toLowerCase()!==t.uuid.toLowerCase())return null;let n=e[r];r+=1,r+=n;let c=e[r];if(r+=1,c!==1)return null;let o=e[r]<<8|e[r+1];r+=2;let s=e[r];r+=1;let i="";if(s===1)i=`${e[r]}.${e[r+1]}.${e[r+2]}.${e[r+3]}`,r+=4;else if(s===2){let p=e[r];r+=1,i=new TextDecoder().decode(e.slice(r,r+p)),r+=p}else if(s===3)i=Gt(e.slice(r,r+16)),r+=16;else return null;return{address:i,port:o,payload:e.slice(r)}}catch{return null}}function Mt(e){let t=[...e].map(r=>r.toString(16).padStart(2,"0")).join("");return`${t.slice(0,8)}-${t.slice(8,12)}-${t.slice(12,16)}-${t.slice(16,20)}-${t.slice(20)}`}function Gt(e){let t=[];for(let r=0;r<16;r+=2)t.push((e[r]<<8|e[r+1]).toString(16));return t.join(":")}import{connect as Jt}from"cloudflare:sockets";async function wt(e,t,r,a){let n=K(e);n.binaryType="arraybuffer";let c=e.headers.get("sec-websocket-protocol")||"",{stream:o,removeEarlyData:s}=Y(n,c,()=>{}),i=d=>{try{n.send(d)}catch{}},p=0,l=0;return o.pipeTo(new WritableStream({async write(d){let u=null;try{let f=s(),A=f&&f.length?jt(f,d):d,x=_t(t.password),h=zt(A,x);if(!h){n.close();return}if(u=Jt({hostname:h.address,port:h.port}),h.payload.length){let w=u.writable.getWriter();await w.write(h.payload),w.releaseLock(),p+=h.payload.length}o.pipeTo(new WritableStream({async write(w){p+=w.byteLength;let k=u.writable.getWriter();try{await k.write(w)}finally{k.releaseLock()}}})).catch(()=>{try{u.close()}catch{}}),u.readable.pipeTo(new WritableStream({write(w){l+=w.byteLength,i(w)}})).catch(()=>{try{n.close()}catch{}}).finally(()=>{C(a,t.id,p,l).catch(()=>{})})}catch{try{u?.close()}catch{}try{n.close()}catch{}C(a,t.id,p,l).catch(()=>{})}}})).catch(()=>{try{n.close()}catch{}}),new Response(null,{status:101,webSocket:n})}function jt(e,t){let r=new Uint8Array(e.length+t.length);return r.set(e),r.set(t,e.length),r}function zt(e,t){try{let r=0,a=new TextDecoder().decode(e.slice(r,r+56));if(r+=56,a!==t||e[r]!==13||e[r+1]!==10)return null;r+=2;let n=e[r];if(r+=1,n!==1)return null;let c=e[r];r+=1;let o="";if(c===1)o=`${e[r]}.${e[r+1]}.${e[r+2]}.${e[r+3]}`,r+=4;else if(c===3){let i=e[r];r+=1,o=new TextDecoder().decode(e.slice(r,r+i)),r+=i}else if(c===4)o=Zt(e.slice(r,r+16)),r+=16;else return null;let s=e[r]<<8|e[r+1];return r+=2,e[r]!==13||e[r+1]!==10?null:(r+=2,{address:o,port:s,payload:e.slice(r)})}catch{return null}}function Zt(e){let t=[];for(let r=0;r<16;r+=2)t.push((e[r]<<8|e[r+1]).toString(16));return t.join(":")}var Xt=new Uint32Array([1116352408,1899447441,3049323471,3921009573,961987163,1508970993,2453635748,2870763221,3624381080,310598401,607225278,1426881987,1925078388,2162078206,2614888103,3248222580,3835390401,4022224774,264347078,604807628,770255983,1249150122,1555081692,1996064986,2554220882,2821834349,2952996808,3210313671,3336571891,3584528711,113926993,338241895,666307205,773529912,1294757372,1396182291,1695183700,1986661051,2177026350,2456956037,2730485921,2820302411,3259730800,3345764771,3516065817,3600352804,4094571909,275423344,430227734,506948616,659060556,883997877,958139571,1322822218,1537002063,1747873779,1955562222,2024104815,2227730452,2361852424,2428436474,2756734187,3204031479,3329325298]);function y(e,t){return e>>>t|e<<32-t}function $t(e){let t=new Uint32Array([3238371032,914150663,812702999,4144912697,4290775857,1750603025,1694076839,3204075428]),r=e.length,a=Math.floor(r/536870912),n=r<<3>>>0,c=new Uint8Array((r+8>>6)+1<<6);c.set(e),c[r]=128;let o=new DataView(c.buffer);o.setUint32(c.length-8,a),o.setUint32(c.length-4,n);let s=new Uint32Array(64);for(let l=0;l<c.length;l+=64){for(let m=0;m<16;m++)s[m]=o.getUint32(l+m*4);for(let m=16;m<64;m++){let F=y(s[m-15],7)^y(s[m-15],18)^s[m-15]>>>3,T=y(s[m-2],17)^y(s[m-2],19)^s[m-2]>>>10;s[m]=s[m-16]+F+s[m-7]+T>>>0}let[d,u,f,A,x,h,w,k]=t;for(let m=0;m<64;m++){let F=y(x,6)^y(x,11)^y(x,25),T=x&h^~x&w,tt=k+F+T+Xt[m]+s[m]>>>0,Qt=y(d,2)^y(d,13)^y(d,22),Lt=d&u^d&f^u&f,qt=Qt+Lt>>>0;k=w,w=h,h=x,x=A+tt>>>0,A=f,f=u,u=d,d=tt+qt>>>0}t[0]=t[0]+d>>>0,t[1]=t[1]+u>>>0,t[2]=t[2]+f>>>0,t[3]=t[3]+A>>>0,t[4]=t[4]+x>>>0,t[5]=t[5]+h>>>0,t[6]=t[6]+w>>>0,t[7]=t[7]+k>>>0}let i=new Uint8Array(28),p=new DataView(i.buffer);for(let l=0;l<7;l++)p.setUint32(l*4,t[l]);return i}function _t(e){return[...$t(new TextEncoder().encode(e))].map(t=>t.toString(16).padStart(2,"0")).join("")}var bt="https://api.cloudflare.com/client/v4";async function H(e,t,r){return(await fetch(bt+t,{...r,headers:{authorization:`Bearer ${e}`,...r?.headers||{}}})).json()}async function yt(e,t,r){let a=await H(e,`/accounts/${t}/storage/kv/namespaces?per_page=100`);for(let n of a?.result||[])if(r.includes(n.title))return n.id;return null}async function At(e,t,r,a,n){let c="----NikaNetBoundary"+Math.random().toString(16).slice(2),s=[`--${c}\r
...
<div class="wrap">
  ...
  <section class="card">
    ...
    <div class="stats">
      <div class="stat"><b>${V(e.quota)} GB</b><span>\u062D\u062C\u0645 \u06A9\u0644</span></div>
      <div class="stat"><b>${V(e.used)} GB</b><span>\u0645\u0635\u0631\u0641 \u0634\u062F\u0647</span></div>
      <div class="stat"><b>${V(e.days)}</b><span>\u0631\u0648\u0632 \u0627\u0639\u062A\u0628\u0627\u0631</span></div>
    </div>
    <div class="protos">${u.map(A=>`<i>${A}</i>`).join("")}</div>
  </section>
  ...
  <section class="card">
    ...
    <div class="linkbox">
      <input id="rawLink" readonly value="${S(t)}"/>
  ...
  <section class="card">
    ...
    <div class="apps">
      <a class="app" href="${c}"><span class="go">\u26A1</span><img src="${vt}" alt="V2rayNG"/><b>V2rayNG</b><em>\u0627\u0646\u062F\u0631\u0648\u06CC\u062F</em></a>
      <a class="app" href="${s}"><span class="go">\u26A1</span><img src="${It}" alt="Hiddify"/><b>Hiddify</b><em>\u0647\u0645\u0647\u200C\u06CC \u067E\u0644\u062A\u0641\u0631\u0645\u200C\u0647\u0627</em></a>
      <a class="app" href="${i}"><span class="go">\u26A1</span><img src="${kt}" alt="Happ"/><b>Happ</b><em>\u0627\u0646\u062F\u0631\u0648\u06CC\u062F \xB7 iOS</em></a>
      <a class="app" href="${o}"><span class="go">\u26A1</span><img src="${St}" alt="V2Box"/><b>V2Box</b><em>iOS \xB7 macOS</em></a>
    </div>
  </section>
  ...
  <footer class="foot">\u0633\u0627\u062E\u062A\u0647\u200C\u0634\u062F\u0647 \u0628\u0627 <b>NIKA NET</b> \xB7 \u0646\u0633\u062E\u0647 ${S(e.version)}<br/>\u0644\u06CC\u0646\u06A9 \u0633\u0627\u0628\u062A \u0631\u0648 \u062C\u0627\u06CC\u06CC \u0627\u0645\u0646 \u0646\u06AF\u0647 \u062F\u0627\u0631 \u{1F510}</footer>
</div>
