var U={title:"Nika Net",host:"nika.example.workers.dev",sni:"www.speedtest.net",wsPath:"/nika-ws",cleanIps:["1.0.0.1","104.16.132.229","188.114.96.9"],protocols:{vless:!0,trojan:!0,warp:!0},adminPassHash:null,secretPath:"nika-admin",sessionSecret:""};var N=new Map,rt=new Map;async function Dt(t,e){try{let r=await t.NIKA_DB.prepare("SELECT value FROM kv WHERE key = ?1").bind(e).first();return r?r.value:null}catch{return null}}async function Kt(t,e,r){try{await t.NIKA_DB.prepare("INSERT INTO kv (key, value) VALUES (?1, ?2) ON CONFLICT(key) DO UPDATE SET value = excluded.value").bind(e,r).run()}catch{}}async function q(t,e){if(N.has(e))return N.get(e);let r=null;return t.NIKA_DB?r=await Dt(t,e):t.NIKA_KV?r=await t.NIKA_KV.get(e):r=rt.get(e)??null,r!==null&&N.set(e,r),r}async function R(t,e,r){N.set(e,r),t.NIKA_DB?await Kt(t,e,r):t.NIKA_KV?await t.NIKA_KV.put(e,r):rt.set(e,r)}async function nt(t){let e=await q(t,"settings"),r={...U};if(e)try{Object.assign(r,JSON.parse(e))}catch{}return r.sessionSecret||(r.sessionSecret=crypto.randomUUID(),await R(t,"settings",JSON.stringify(r))),r}async function W(t,e){await R(t,"settings",JSON.stringify(e))}async function y(t){let e=await q(t,"users");if(!e)return[];try{return JSON.parse(e)}catch{return[]}}async function v(t,e){await R(t,"users",JSON.stringify(e))}async function B(t,e){let r=await q(t,e),o=parseInt(r||"0",10);return isNaN(o)?0:o}async function P(t,e,r){let n=await B(t,e)+r;return await R(t,e,String(n)),n}async function M(t,e){let r=await q(t,e);if(!r)return null;try{return JSON.parse(r)}catch{return null}}async function ot(t,e,r){await R(t,e,JSON.stringify(r))}var G=new TextEncoder;async function J(t){let e=await crypto.subtle.digest("SHA-256",G.encode(t));return[...new Uint8Array(e)].map(r=>r.toString(16).padStart(2,"0")).join("")}function at(){return[...crypto.getRandomValues(new Uint8Array(24))].map(e=>e.toString(16).padStart(2,"0")).join("")}async function j(t,e){let r=await crypto.subtle.importKey("raw",G.encode(t),{name:"HMAC",hash:"SHA-256"},!1,["sign"]),o=await crypto.subtle.sign("HMAC",r,G.encode(e));return e+"."+[...new Uint8Array(o)].map(n=>n.toString(16).padStart(2,"0")).join("")}async function Ot(t,e){let r=e.lastIndexOf(".");if(r<0)return!1;let o=e.slice(0,r),c=(await j(t,o)).split(".")[1],a=e.slice(r+1);if(c.length!==a.length)return!1;let s=0;for(let i=0;i<c.length;i++)s|=c.charCodeAt(i)^a.charCodeAt(i);return s===0}async function it(t,e){let r=(t.headers.get("Cookie")||"").split(";").map(n=>n.trim()).find(n=>n.startsWith("nika_session="));if(!r)return!1;let o=decodeURIComponent(r.split("=").slice(1).join("="));return Ot(e.sessionSecret,o)}var z="nika_session";function D(t){let e=(t.cleanIps||[]).filter(Boolean);return e.length?e[Math.floor(Math.random()*e.length)]:t.host}function Vt(t,e){let r=D(e),o=new URLSearchParams({encryption:"none",security:"tls",sni:e.host,fp:"chrome",type:"ws",host:e.host,path:e.wsPath+"?ed=2048&proto=vless"});return`vless://${t.uuid}@${r}:443?${o.toString()}#NikaNet-${encodeURIComponent(t.name)}`}function Ft(t,e){let r=D(e),o=new URLSearchParams({security:"tls",sni:e.host,fp:"chrome",type:"ws",host:e.host,path:e.wsPath+"?ed=2048&proto=trojan"});return`trojan://${t.password}@${r}:443?${o.toString()}#NikaNet-${encodeURIComponent(t.name)}`}function K(t,e){let r=[];return e.protocols.vless&&r.push(Vt(t,e)),e.protocols.trojan&&r.push(Ft(t,e)),btoa(r.join(`
`)).replace(/=+$/,"")}function Z(t,e){let r=D(e),o=[],n=`# Nika Net \u2014 ${t.name}
mixed-port: 7890
allow-lan: false
mode: rule
log-level: info
...
proxies:
`;return e.protocols.vless&&(o.push("NikaNet-VLESS"),n+=`  - name: "NikaNet-VLESS"
    type: vless
    server: ${r}
    port: 443
    uuid: ${t.uuid}
    ...
    ws-opts:
      ...
      headers: { Host: "${e.host}" }
`),e.protocols.trojan&&(o.push("NikaNet-Trojan"),n+=`  - name: "NikaNet-Trojan"
    type: trojan
    server: ${r}
    port: 443
    password: ${t.password}
...
`),n+=`proxy-groups:
  - name: "NikaNet"
    type: select
    proxies: [${o.map(c=>`"${c}"`).join(", ")}]
`,n+=`rules:
  - GEOIP,IR,DIRECT
  - MATCH,NikaNet
`,n}function X(t,e){let r=D(e),o=[],n=[];e.protocols.vless&&(n.push("NikaNet-VLESS"),o.push({tag:"NikaNet-VLESS",type:"vless",server:r,server_port:443,uuid:t.uuid,network:"ws",tls:{enabled:!0,server_name:e.host,utls:{enabled:!0,fingerprint:"chrome"}},transport:{type:"ws",path:e.wsPath+"?ed=2048&proto=vless",headers:{Host:e.host}}})),e.protocols.trojan&&(n.push("NikaNet-Trojan"),o.push({tag:"NikaNet-Trojan",type:"trojan",server:r,server_port:443,password:t.password,network:"ws",tls:{enabled:!0,server_name:e.host,utls:{enabled:!0,fingerprint:"chrome"}},transport:{type:"ws",path:e.wsPath+"?ed=2048&proto=trojan",headers:{Host:e.host}}}));let c={log:{level:"info"},dns:{servers:[{tag:"cf",address:"https://1.1.1.1/dns-query",detour:"select"}]},outbounds:o.concat([{tag:"select",type:"selector",outbounds:n},{tag:"direct",type:"direct"}]),route:{rules:[{geoip:"ir",outbound:"direct"}],final:"select"}};return JSON.stringify(c,null,2)}function Tt(t){let e=(t.uuid.replace(/-/g,"")+(t.password||"")).slice(0,64).padEnd(64,"0"),r=new Uint8Array(32);for(let n=0;n<32;n++)r[n]=parseInt(e.slice(n*2,n*2+2),16)||0;let o="";for(let n=0;n<32;n++)o+=String.fromCharCode(r[n]);return btoa(o)}function ct(t){return`[Interface]
PrivateKey = ${Tt(t)}
...
`}var E=new Map,lt=new Map,pt=0,C=()=>{let t=new Date,e=String(t.getMonth()+1).padStart(2,"0"),r=String(t.getDate()).padStart(2,"0");return`${t.getFullYear()}-${e}-${r}`};function dt(){E.set("req:total",(E.get("req:total")||0)+1),E.set("req:"+C(),(E.get("req:"+C())||0)+1)}async function gt(t){let e=Date.now();if(!(e-pt<15e3)){pt=e;for(let[r,o]of E)o&&await P(t,r,o);E.clear()}}async function ut(t){return await B(t,"req:"+C())+(E.get("req:"+C())||0)}async function ft(t){return await B(t,"req:total")+(E.get("req:total")||0)}async function H(t,e,r,o){if(!r&&!o)return;let n=await y(t),c=n.find(s=>s.id===e);c&&(c.used=(c.used||0)+(r+o)/1e9,await v(t,n)),await P(t,"traffic:"+C(),r+o);let a=lt.get(e)||0;Date.now()-a>6e4&&(lt.set(e,Date.now()),await I(t,{icon:"\u{1F4E1}",text:`\u0627\u062A\u0635\u0627\u0644 \u062C\u062F\u06CC\u062F${c?` \u2014 ${c.name}`:""}`,time:Date.now()}))}async function mt(t){let e=[];for(let r=6;r>=0;r--){let o=new Date;o.setDate(o.getDate()-r);let n=String(o.getMonth()+1).padStart(2,"0"),c=String(o.getDate()).padStart(2,"0"),a=`${o.getFullYear()}-${n}-${c}`,s=await B(t,"traffic:"+a);e.push({date:a.slice(5),gb:Math.round(s/1e9*100)/100})}return e}async function I(t,e){let r=await M(t,"activity")||[];r.unshift(e),r.length>40&&(r.length=40),await ot(t,"activity",r)}async function ht(t){return await M(t,"activity")||[]}import{connect as Pt}from"cloudflare:sockets";function O(t,e,r){let o=null;try{e&&(o=Uint8Array.from(atob(e),s=>s.charCodeAt(0)))}catch{o=null}let n=null,c=new Promise(s=>n=s);return t.addEventListener("message",()=>{n&&(n(),n=null)}),t.addEventListener("error",()=>{n&&(n(),n=null)}),{stream:new ReadableStream({start(s){let i=p=>{let l=p.data;l instanceof ArrayBuffer?s.enqueue(new Uint8Array(l)):Array.isArray(l)?s.enqueue(new Uint8Array(l)):typeof l=="string"&&s.enqueue(new TextEncoder().encode(l))};t.addEventListener("message",i),t.addEventListener("close",()=>{try{s.close()}catch{}}),t.addEventListener("error",()=>{try{s.error(new Error("ws error"))}catch{}})},async pull(){await c},cancel(){try{t.close()}catch{}}}),removeEarlyData:()=>{let s=o;return o=null,s}}}function Y(t){let e=new WebSocketPair,[r,o]=Object.values(e);return o.accept(),o}function g(t,e=200){return new Response(JSON.stringify(t),{status:e,headers:{"content-type":"application/json; charset=utf-8","access-control-allow-origin":"*"}})}async function wt(t,e,r,o){let n=Y(t);n.binaryType="arraybuffer";let c=t.headers.get("sec-websocket-protocol")||"",{stream:a,removeEarlyData:s}=O(n,c,()=>{}),i=d=>{try{n.send(d)}catch{}},p=0,l=0;return a.pipeTo(new WritableStream({async write(d){let u=null;try{let w=s(),b=w&&w.length?Mt(w,d):d,m=Gt(b,e);if(!m){n.close();return}if(u=Pt({hostname:m.address,port:m.port}),i(new Uint8Array([0,0])),m.payload.length){let f=u.writable.getWriter();await f.write(m.payload),f.releaseLock(),p+=m.payload.length}a.pipeTo(new WritableStream({async write(f){p+=f.byteLength;let h=u.writable.getWriter();try{await h.write(f)}finally{h.releaseLock()}}})).catch(()=>{try{u.close()}catch{}}),u.readable.pipeTo(new WritableStream({write(f){l+=f.byteLength,i(f)}})).catch(()=>{try{n.close()}catch{}}).finally(()=>{xt(o,e.id,p,l)})}catch{try{u?.close()}catch{}try{n.close()}catch{}xt(o,e.id,p,l)}}})).catch(()=>{try{n.close()}catch{}}),new Response(null,{status:101,webSocket:n})}function xt(t,e,r,o){H(t,e,r,o).catch(()=>{})}function Mt(t,e){let r=new Uint8Array(t.length+e.length);return r.set(t),r.set(e,t.length),r}function Gt(t,e){try{let r=0;if(t[r]!==0)return null;r+=1;let o=Jt(t.slice(r,r+16));if(r+=16,o.toLowerCase()!==e.uuid.toLowerCase())return null;let n=t[r];r+=1,r+=n;let c=t[r];if(r+=1,c!==1)return null;let a=t[r]<<8|t[r+1];r+=2;let s=t[r];r+=1;let i="";if(s===1)i=`${t[r]}.${t[r+1]}.${t[r+2]}.${t[r+3]}`,r+=4;else if(s===2){let p=t[r];r+=1,i=new TextDecoder().decode(t.slice(r,r+p)),r+=p}else if(s===3)i=jt(t.slice(r,r+16)),r+=16;else return null;return{address:i,port:a,payload:t.slice(r)}}catch{return null}}function Jt(t){let e=[...t].map(r=>r.toString(16).padStart(2,"0")).join("");return`${e.slice(0,8)}-${e.slice(8,12)}-${e.slice(12,16)}-${e.slice(16,20)}-${e.slice(20)}`}function jt(t){let e=[];for(let r=0;r<16;r+=2)e.push((t[r]<<8|t[r+1]).toString(16));return e.join(":")}import{connect as zt}from"cloudflare:sockets";async function bt(t,e,r,o){let n=Y(t);n.binaryType="arraybuffer";let c=t.headers.get("sec-websocket-protocol")||"",{stream:a,removeEarlyData:s}=O(n,c,()=>{}),i=d=>{try{n.send(d)}catch{}},p=0,l=0;return a.pipeTo(new WritableStream({async write(d){let u=null;try{let w=s(),b=w&&w.length?Zt(w,d):d,m=ee(e.password),f=Xt(b,m);if(!f){n.close();return}if(u=zt({hostname:f.address,port:f.port}),f.payload.length){let h=u.writable.getWriter();await h.write(f.payload),h.releaseLock(),p+=f.payload.length}a.pipeTo(new WritableStream({async write(h){p+=h.byteLength;let k=u.writable.getWriter();try{await k.write(h)}finally{k.releaseLock()}}})).catch(()=>{try{u.close()}catch{}}),u.readable.pipeTo(new WritableStream({write(h){l+=h.byteLength,i(h)}})).catch(()=>{try{n.close()}catch{}}).finally(()=>{H(o,e.id,p,l).catch(()=>{})})}catch{try{u?.close()}catch{}try{n.close()}catch{}H(o,e.id,p,l).catch(()=>{})}}})).catch(()=>{try{n.close()}catch{}}),new Response(null,{status:101,webSocket:n})}function Zt(t,e){let r=new Uint8Array(t.length+e.length);return r.set(t),r.set(e,t.length),r}function Xt(t,e){try{let r=0,o=new TextDecoder().decode(t.slice(r,r+56));if(r+=56,o!==e||t[r]!==13||t[r+1]!==10)return null;r+=2;let n=t[r];if(r+=1,n!==1)return null;let c=t[r];r+=1;let a="";if(c===1)a=`${t[r]}.${t[r+1]}.${t[r+2]}.${t[r+3]}`,r+=4;else if(c===3){let i=t[r];r+=1,a=new TextDecoder().decode(t.slice(r,r+i)),r+=i}else if(c===4)a=$t(t.slice(r,r+16)),r+=16;else return null;let s=t[r]<<8|t[r+1];return r+=2,t[r]!==13||t[r+1]!==10?null:(r+=2,{address:a,port:s,payload:t.slice(r)})}catch{return null}}function $t(t){let e=[];for(let r=0;r<16;r+=2)e.push((t[r]<<8|t[r+1]).toString(16));return e.join(":")}var _t=new Uint32Array([1116352408,1899447441,3049323471,3921009573,961987163,1508970993,2453635748,2870763221,3624381080,310598401,607225278,1426881987,1925078388,2162078206,2614888103,3248222580,3835390401,4022224774,264347078,604807628,770255983,1249150122,1555081692,1996064986,2554220882,2821834349,2952996808,3210313671,3336571891,3584528711,113926993,338241895,666307205,773529912,1294757372,1396182291,1695183700,1986661051,2177026350,2456956037,2730485921,2820302411,3259730800,3345764771,3516065817,3600352804,4094571909,275423344,430227734,506948616,659060556,883997877,958139571,1322822218,1537002063,1747873779,1955562222,2024104815,2227730452,2361852424,2428436474,2756734187,3204031479,3329325298]);function A(t,e){return t>>>e|t<<32-e}function te(t){let e=new Uint32Array([3238371032,914150663,812702999,4144912697,4290775857,1750603025,1694076839,3204075428]),r=t.length,o=Math.floor(r/536870912),n=r<<3>>>0,c=new Uint8Array((r+8>>6)+1<<6);c.set(t),c[r]=128;let a=new DataView(c.buffer);a.setUint32(c.length-8,o),a.setUint32(c.length-4,n);let s=new Uint32Array(64);for(let l=0;l<c.length;l+=64){for(let x=0;x<16;x++)s[x]=a.getUint32(l+x*4);for(let x=16;x<64;x++){let F=A(s[x-15],7)^A(s[x-15],18)^s[x-15]>>>3,T=A(s[x-2],17)^A(s[x-2],19)^s[x-2]>>>10;s[x]=s[x-16]+F+s[x-7]+T>>>0}let[d,u,w,b,m,f,h,k]=e;for(let x=0;x<64;x++){let F=A(m,6)^A(m,11)^A(m,25),T=m&f^~m&h,et=k+F+T+_t[x]+s[x]>>>0,Lt=A(d,2)^A(d,13)^A(d,22),Nt=d&u^d&w^u&w,qt=Lt+Nt>>>0;k=h,h=f,f=m,m=b+et>>>0,b=w,w=u,u=d,d=et+qt>>>0}e[0]=e[0]+d>>>0,e[1]=e[1]+u>>>0,e[2]=e[2]+w>>>0,e[3]=e[3]+b>>>0,e[4]=e[4]+m>>>0,e[5]=e[5]+f>>>0,e[6]=e[6]+h>>>0,e[7]=e[7]+k>>>0}let i=new Uint8Array(28),p=new DataView(i.buffer);for(let l=0;l<7;l++)p.setUint32(l*4,e[l]);return i}function ee(t){return[...te(new TextEncoder().encode(t))].map(e=>e.toString(16).padStart(2,"0")).join("")}var yt="https://api.cloudflare.com/client/v4";async function Q(t,e,r){return(await fetch(yt+e,{...r,headers:{authorization:`Bearer ${t}`,...r?.headers||{}}})).json()}async function At(t,e,r){let o=await Q(t,`/accounts/${e}/storage/kv/namespaces?per_page=100`);for(let n of o?.result||[])if(r.includes(n.title))return n.id;return null}async function Et(t,e,r,o,n){let c="----NikaNetBoundary"+Math.random().toString(16).slice(2),s=[`--${c}\r
...
<div class="wrap">
  ...
  <header class="hero">
    ...
    <div class="chip ${d}"><span class="b"></span>${l}</div>
  </header>

  <section class="card">
    <h2>\u{1F4CA} \u0648\u0636\u0639\u06CC\u062A \u0627\u0634\u062A\u0631\u0627\u06A9</h2>
    <div class="meter"><i style="width:${p}%"></i></div>
    <div class="mlabel"><span>\u0645\u0635\u0631\u0641</span><span>${V(p)}\u066A</span></div>
    <div class="stats">
    ...
    <div class="protos">${u.map(b=>`<i>${b}</i>`).join("")}</div>
  </section>
  ...
  <section class="card">
    ...
    <div class="apps">
      <a class="app" href="${c}"><span class="go">\u26A1</span><img src="${St}" alt="V2rayNG"/><b>V2rayNG</b><em>\u0627\u0646\u062F\u0631\u0648\u06CC\u062F</em></a>
      <a class="app" href="${s}"><span class="go">\u26A1</span><img src="${kt}" alt="Hiddify"/><b>Hiddify</b><em>\u0647\u0645\u0647\u200C\u06CC \u067E\u0644\u062A\u0641\u0631\u0645\u200C\u0647\u0627</em></a>
      <a class="app" href="${i}"><span class="go">\u26A1</span><img src="${vt}" alt="Happ"/><b>Happ</b><em>\u0627\u0646\u062F\u0631\u0648\u06CC\u062F \xB7 iOS</em></a>
      <a class="app" href="${a}"><span class="go">\u26A1</span><img src="${Ut}" alt="V2Box"/><b>V2Box</b><em>iOS \xB7 macOS</em></a>
    </div>
  </section>
...
`,L="0.5.2",Ct=(t,e=200)=>new Response(t,{status:e,headers:{"content-type":"text/html; charset=utf-8"}});function oe(t){return t.headers.set("access-control-allow-origin","*"),t}var qe={async fetch(t,e,r){try{dt(),r.waitUntil(gt(e));let o=new URL(t.url),n=o.pathname,c=await nt(e);if((t.headers.get("Upgrade")||"").toLowerCase()==="websocket")return se(t,e,c);if(n==="/admin"||n==="/admin/")return Ct(Bt);if(n.startsWith("/api/"))return oe(await ae(t,e,c,o));if(n.startsWith("/sub/"))return ie(t,e,c,n);let a=n.match(/^\/([0-9a-fA-F-]{36})\/?$/);return a?ce(t,e,c,a[1]):n==="/health"?g({ok:!0,name:c.title}):c.host===U.host?Ct(Bt):Response.redirect("https://www.cloudflare.com",302)}catch(o){return g({error:String(o)},500)}}};async function se(t,e,r){let o=new URL(t.url),n=o.searchParams.get("proto")||t.headers.get("x-nika-proto")||"",c=await y(e),a=o.searchParams.get("uuid")||"",s=c.find(i=>i.uuid.toLowerCase()===a.toLowerCase());return s?s.active?n==="trojan"?bt(t,s,r,e):wt(t,s,r,e):g({error:"user inactive"},403):g({error:"no user for this uuid"},403)}async function ae(t,e,r,o){let n=o.pathname.replace("/api/",""),c=t.method.toUpperCase();if(n==="info")return g({name:r.title,setup:!r.adminPassHash,protocols:r.protocols,version:L});if(n==="update/check"){let a=await le();return g({current:L,latest:a.version,notes:a.notes||"",upToDate:pe(L,a.version)>=0})}if(n==="login"&&c==="POST"){let s=(await t.json().catch(()=>({}))).password||"",i=!r.adminPassHash;if(i){if(!s||s.length<4)return g({error:"password too short"},400);r.adminPassHash=await J(s),await W(e,r)}if(!(r.adminPassHash===await J(s)))return g({error:"wrong password"},401);let l=await j(r.sessionSecret,JSON.stringify({t:Date.now()}));await I(e,{icon:i?"\u{1F6E0}":"\u{1F510}",text:i?"\u0646\u0635\u0628 \u0627\u0648\u0644\u06CC\u0647 \u067E\u0646\u0644 \u2014 \u0631\u0645\u0632 \u0627\u062F\u0645\u06CC\u0646 \u062B\u0628\u062A \u0634\u062F":"\u0648\u0631\u0648\u062F \u0627\u062F\u0645\u06CC\u0646 \u0628\u0647 \u067E\u0646\u0644",time:Date.now()});let d=g({ok:!0,setup:i});return d.headers.set("set-cookie",`${z}=${encodeURIComponent(l)}; HttpOnly; Path=/; Max-Age=86400; SameSite=Lax`),d}if(n==="logout"){let a=g({ok:!0});return a.headers.set("set-cookie",`${z}=; HttpOnly; Path=/; Max-Age=0`),a}if(!await it(t,r))return g({error:"unauthorized"},401);switch(n){case"status":{let a=await y(e);return g({title:r.title,setup:!1,users:a.length,active:a.filter(s=>s.active).length,usedGb:Math.round(a.reduce((s,i)=>s+(i.used||0),0)*100)/100,requestsToday:await ut(e),requestsTotal:await ft(e),protocols:r.protocols,activity:await ht(e),traffic7d:await mt(e)})}case"users":{let a=await y(e);if(c==="GET")return g(a);if(c==="POST"){let s=await t.json().catch(()=>({})),i={id:crypto.randomUUID(),name:s.name||"\u06A9\u0627\u0631\u0628\u0631",uuid:crypto.randomUUID(),password:at(),quota:Number(s.quota)||50,used:0,days:Number(s.days)||30,active:!0,createdAt:Date.now()};return a.push(i),await v(e,a),await I(e,{icon:"\u{1F464}",text:`\u06A9\u0627\u0631\u0628\u0631 \u0633\u0627\u062E\u062A\u0647 \u0634\u062F \u2014 ${i.name}`,time:Date.now()}),g(i)}if(c==="DELETE"){let s=o.searchParams.get("id"),i=a.find(l=>l.id===s),p=a.filter(l=>l.id!==s);return await v(e,p),await I(e,{icon:"\u{1F5D1}",text:`\u06A9\u0627\u0631\u0628\u0631 \u062D\u0630\u0641 \u0634\u062F \u2014 ${i?.name||s}`,time:Date.now()}),g({ok:!0})}break}case"users/toggle":{if(c!=="POST")break;let a=await t.json().catch(()=>({})),s=await y(e),i=s.find(p=>p.id===a.id);return i?(i.active=!i.active,await v(e,s),await I(e,{icon:i.active?"\u{1F7E2}":"\u26D4",text:`${i.name} ${i.active?"\u0641\u0639\u0627\u0644":"\u063A\u06CC\u0631\u0641\u0639\u0627\u0644"} \u0634\u062F`,time:Date.now()}),g({ok:!0,active:i.active})):g({error:"not found"},404)}case"settings":{if(c==="GET")return g(r);if(c==="POST"){let a=await t.json().catch(()=>({})),s={...r};return typeof a.title=="string"&&(s.title=a.title),typeof a.host=="string"&&(s.host=a.host),typeof a.sni=="string"&&(s.sni=a.sni),typeof a.wsPath=="string"&&(s.wsPath=a.wsPath),Array.isArray(a.cleanIps)&&(s.cleanIps=a.cleanIps),a.protocols&&(s.protocols={...r.protocols,...a.protocols}),await W(e,s),await I(e,{icon:"\u2699\uFE0F",text:"\u062A\u0646\u0638\u06CC\u0645\u0627\u062A \u067E\u0646\u0644 \u0628\u0647\u200C\u0631\u0648\u0632\u0631\u0633\u0627\u0646\u06CC \u0634\u062F",time:Date.now()}),g({ok:!0})}break}case"gen":{let a=o.searchParams.get("id"),i=(await y(e)).find(l=>l.id===a);if(!i)return g({error:"user not found"},404);let p={...r,host:tt(t,r)};return g({user:{id:i.id,name:i.name,quota:i.quota,used:Math.round((i.used||0)*100)/100,days:i.days,active:i.active},base64:K(i,p),clash:Z(i,p),singbox:X(i,p),warp:r.protocols.warp?ct(i):null})}case"update/apply":{if(c!=="POST")break;let a=await t.json().catch(()=>({})),s=await de(e,r,a.token||"");return g(s,s.ok?200:400)}}return g({error:"not found"},404)}function tt(t,e){let r=(e.host||"").trim();return r&&r!==U.host?r:new URL(t.url).hostname}async function ie(t,e,r,o){let n=o.replace("/sub/",""),c=n.split("/")[0].split(".")[0],a=n.split(".").pop()?.toLowerCase()||"",i=(await y(e)).find(h=>h.password===c||h.uuid.replace(/-/g,"").slice(0,12)===c);if(!i)return g({error:"invalid token"},404);let p=t.headers.get("Accept")||"",l=t.headers.get("Sec-Fetch-Dest")||"",d=t.headers.get("Sec-Fetch-Mode")||"",u=l==="document"||d==="navigate";if(p.includes("text/html")&&u){let h=new URL(t.url).origin;return new Response(Rt({name:i.name,active:!!i.active,quota:Number(i.quota)||0,used:Math.round((i.used||0)*100)/100,days:Number(i.days)||0,origin:h,token:c,version:L,protocols:r.protocols}),{headers:{"content-type":"text/html; charset=utf-8"}})}let w=a==="yaml"||a==="yml",b=a==="json",m={...r,host:tt(t,r)},f=w?Z(i,m):b?X(i,m):K(i,m);return new Response(f,{headers:{"content-type":w?"text/yaml":b?"application/json":"text/plain"}})}async function ce(t,e,r,o){let c=(await y(e)).find(i=>i.uuid.toLowerCase()===o.toLowerCase());if(!c)return g({error:"unknown uuid"},404);let a={...r,host:tt(t,r)},s=K(c,a);return new Response(s,{headers:{"content-type":"text/plain"}})}var Qt="https://raw.githubusercontent.com/NikaTeem/Nika-Net/main",_=null,Ht=0;async function le(){if(_&&Date.now()-Ht<3e5)return _;try{let t=await fetch(`${Qt}/version.json`,{cf:{cacheTtl:300}});if(!t.ok)throw new Error("fetch failed");let e=await t.json();return _=e,Ht=Date.now(),e}catch{return{version:L,notes:""}}}function pe(t,e){let r=t.split(".").map(n=>parseInt(n,10)||0),o=e.split(".").map(n=>parseInt(n,10)||0);for(let n=0;n<3;n++){let c=(r[n]||0)-(o[n]||0);if(c!==0)return c}return 0}async function de(t,e,r){if(!r||r.length<20)return{ok:!1,error:"token required"};let o=await Q(r,"/user/tokens/verify");if(!o?.success)return{ok:!1,error:o?.errors?.[0]?.message||"\u062A\u0648\u06A9\u0646 \u0646\u0627\u0645\u0639\u062A\u0628\u0631 \u0627\u0633\u062A"};let c=(await Q(r,"/accounts?per_page=50"))?.result?.[0]?.id;if(!c)return{ok:!1,error:"\u0627\u06A9\u0627\u0646\u062A\u06CC \u0628\u0627 \u0627\u06CC\u0646 \u062A\u0648\u06A9\u0646 \u067E\u06CC\u062F\u0627 \u0646\u0634\u062F"};let a=(e.host||"").split(".")[0];if(!a)return{ok:!1,error:"\u0627\u0628\u062A\u062F\u0627 Host \u0648\u0631\u06A9\u0631 \u0631\u0627 \u062F\u0631 \u062A\u0646\u0638\u06CC\u0645\u0627\u062A \u0648\u0627\u0631\u062F \u06A9\u0646"};let s=await At(r,c,[`nika-${a}-kv`,`${a}-kv`]),i=await fetch(`${Qt}/dist/worker.js`);if(!i.ok)return{ok:!1,error:"\u062F\u0631\u06CC\u0627\u0641\u062A \u0622\u062E\u0631\u06CC\u0646 \u0646\u0633\u062E\u0647 \u0645\u0645\u06A9\u0646 \u0646\u0634\u062F"};let p=await i.text(),d=await Et(r,c,a,p,s?[{type:"kv_namespace",name:"NIKA_KV",namespace_id:s}]:[]);return d.ok?(await It(r,c,a),await I(t,{icon:"\u{1F504}",text:"\u067E\u0646\u0644 \u0628\u0647\u200C\u0631\u0648\u0632\u0631\u0633\u0627\u0646\u06CC \u0634\u062F",time:Date.now()}),{ok:!0}):{ok:!1,error:d.err}}export{qe as default};
